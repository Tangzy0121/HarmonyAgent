...
const ACCEPTED_CONTENT_TYPES = new Set([
  ...
  // 部分浏览器对 .md/.docx 不给出具体 MIME，退回 octet-stream 时按扩展名分发
...
export function createDocumentsRouter(dependencies: DocumentsRouterDependencies): Router {
  ...
  router.post('/', async (req, res) => {
    ...
    try {
      ...
      else parsed = await parseDocx(req.body)
      ...
      const code = error instanceof PdfParseError || error instanceof TextDocumentError || error instanceof DocxParseError ? error.code : 'pdf_unreadable'
