...
> [!IMPORTANT]
> 当前仓库是 **HarmonyAgent 学习原型的开发基线**，不是已经完成的产品 MVP。仓库包含高保真前端、HarmonyOS 客户端骨架和实验性本地后端；真实模型端到端、正式 API、账户与持久化服务、证据化知识图谱和生产部署仍待后续实现或验证。
...
当前集成范围、能力边界与验收门槛见 [学习原型基线与验收范围](docs/MVP_SCOPE.md)。PR #7 只整理现有原型并建立可构建、可测试的开发基线，不以“Book 或学习 Agent 已达到完成版 MVP”作为结论。

当前代码状态统一使用以下标签：

| 状态 | 含义 |
| --- | --- |
| `Prototype` | 已有页面、实验实现或本地链路，尚未完成真实端到端验收 |
| `Integrated` | 若干模块已按临时接口接通 |
| `Verified` | 已使用真实文件、真实模型和目标客户端完成验证 |
| `Planned` | 已设计但尚未实现 |
| `Deferred` | 当前阶段明确不做 |

现阶段主要能力属于 `Prototype`。自动测试和构建通过只证明工程基线可继续开发；模型调用在测试中主要使用 Mock，不能据此判断真实产品闭环已经可用。

下一阶段先冻结文档、Book、学习证据、概念关系和 Agent 的核心模型与 API，再分别推进正式后端、HarmonyOS 联调和真实端到端验收。长期产品愿景不等同于当前实现状态。

## 工程现状

| 模块 | 当前技术 | 当前定位 |
| --- | --- | --- |
| `admin/` | React 18、TypeScript、Vite、Tailwind CSS | 高保真页面与部分真实接口的 Web 原型；不是正式产品前端。 |
| `entry/` | ArkTS、ArkUI、HarmonyOS | 原生 App、HTTP Repository 与本地能力骨架；尚未完成真实后端端到端验收。 |
| `server/` | Node.js、Express、TypeScript | 包含文档、Book、学习状态和 Agent 的实验性本地实现；不是生产级后端。 |
...
代理默认端口为 `3456`，健康检查为 `http://localhost:3456/health`。现有路由是原型联调的临时接口，不应作为产品 API 定稿。Book 数据主要保存在服务端本地 JSON 和文件中，也不等同于正式数据库、对象存储或多用户持久化。
...
├── server/                # 开发期 LLM 代理骨架
└── docs/
    ├── PRD.md             # v0.2 长期产品愿景草案，待 v0.3 重构
    ├── MVP_SCOPE.md       # 当前原型范围、实现状态与验收真源
...
| 关注点 | 当前真源 |
| --- | --- |
| 当前原型范围、实现状态与验收 | [学习原型基线与验收范围](docs/MVP_SCOPE.md) |
| 长期产品定位与功能讨论 | [产品需求文档（PRD）](docs/PRD.md) |
