...
- 页面共享浅矿物灰画布、深色编辑排版和稳定不透明表面；桃色只用于品牌、Agent 与即时行动，不使用环境光、背景模糊或折射玻璃；
- 地图用于知识结构与节点探索，知识库用于文件与正文，二者不互相替代；
- 代码先实现可替换的状态、模拟数据和接口边界，开放决策不写死在页面逻辑中。

## 下一步

按 [跨平台客户端迁移方案](docs/architecture/跨平台客户端迁移方案.md) 先完成 ArkUI 工程基线与“今日”垂直切片，再逐步迁移 P0/P1 核心闭环。当前 React 原型保留为视觉与交互基准；真实能力接入前，应先完成仓库差距与重构清单，并确认 PRD 中的开放决策。
