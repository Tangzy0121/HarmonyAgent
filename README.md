...
HarmonyAgent 面向持续处理学习资料的大学生，也兼容备考者、职场学习者和自主研究者。它不是网盘、摘要工具或单文档问答：产品要连接“资料摄入 → 结构理解 → 自由探索 → 单点学习 → 学习证据 → 下一步行动”的完整闭环。
...
| 模块 | 当前技术 | 当前定位 |
| --- | --- | --- |
| `admin/` | React 18、TypeScript、Vite、Tailwind CSS | 高保真前端原型的主要实现位置；当前页面仍是占位实现。 |
...
2. `docs/design-system/UI与交互规范.md`；
3. `docs/design-system/视觉设计规范.md`；
4. `docs/design-system/用户旅程.md`；
5. `docs/design-system/信息架构与页面结构.md`；
6. README、历史会议记录、归档方案、现有代码注释与早期实现。

`docs/api/` 是技术参考说明，不定义产品方向。`docs/archive/` 与标注为历史记录的 `docs/local/` 文档仅用于追溯，不应用作新功能的需求来源。
...
- 页面以编辑式平面内容为主，磨砂玻璃仅用于导航、控制和抽屉；
- 地图用于知识结构与节点探索，知识库用于文件与正文，二者不互相替代；
- 代码先实现可替换的状态、模拟数据和接口边界，开放决策不写死在页面逻辑中。

## 下一步

按 [当前实施任务清单](docs/local/项目开发任务清单.md) 完成 P0 四张基准页，随后补齐 P1 的首次闭环。真实能力接入前，应先完成仓库差距与重构清单，并确认 PRD 中的开放决策。
