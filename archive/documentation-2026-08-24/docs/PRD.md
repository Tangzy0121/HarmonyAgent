...
> 原日期：2026-08-03
> 状态更新：2026-08-24
> 当前交付目标：整理可构建、可测试的学习原型基线  
> 文档状态：长期产品设计参考，不是当前实现清单

> [!WARNING]
> 本文已被 [`docs/product/`](product/README.md) 和 [ADR-002](architecture/ADR-002-统一产品口径与单一学习项目闭环.md) 取代，保留用于追溯 2026-08-03 的产品探索。其中“今日｜学习｜知识库”、独立学习地图、文件理解页和 S01—S10 不再是现行实现依据。当前代码状态仍以 [`MVP_SCOPE.md`](MVP_SCOPE.md) 为准。
...
本文后续出现的“首版”“必须”“验收”等表述是产品需求草案，不自动代表当前代码已经具备对应能力。实现状态统一使用：

| 状态 | 含义 |
| --- | --- |
| `Prototype` | 有页面、实验实现或本地链路，尚未完成真实端到端验收 |
| `Integrated` | 部分模块已按临时接口接通 |
| `Verified` | 使用真实文件、真实模型和目标客户端完成验证 |
| `Planned` | 已设计，尚未实现 |
| `Deferred` | 当前阶段明确不做 |

PRD 负责说明“未来希望做什么”，`MVP_SCOPE.md` 负责说明“当前实际上做到哪里”。
...
本 PRD 暂时保留产品定位和长期功能讨论；当前原型的范围、实现状态与验收以 `MVP_SCOPE.md` 为准。待团队确认新的一级入口、Book、Chat、今日学习和知识图方案后，再发布替代本文的 PRD v0.3。当前决策记录见《ADR-001：文档真源与烟晶材质基线》，并按以下关注点执行：

| 关注点 | 当前真源 |
|---|---|
| 长期产品定位与功能讨论 | 本《PRD.md》 |
| 当前原型范围、实现状态与验收 | `MVP_SCOPE.md` |
...
同一关注点冲突时，按“用户最新确认并记录的 ADR → 当前原型范围文档 → 当前专项规范 → 当前实现基准 → 历史记录”处理。本文旧版全局结构曾定义为：

> **今日｜学习｜知识库 + 独立 Agent 入口**

该结构当前标记为 `Planned / Reconsidering`，不是 PR #7 的完成状态。当前讨论中的候选入口是“今日学习｜Chat｜学习书”，Memory 首先作为后端能力，知识图放在学习书内部；在 PRD v0.3 确认前，不以任一方案冻结客户端导航或正式 API。
...
> 本章是 v0.2 的旧版信息架构草案，当前状态为 `Planned / Reconsidering`。它用于比较和追溯，不代表 PR #7 已实现，也不应阻止团队重新确认“今日学习｜Chat｜学习书”的候选结构。
...
> 本章记录原 P0—P2 设想。当前工程阶段统一按 `MVP_SCOPE.md` 的 Prototype Baseline 管理，不再用 P0、P1、P2 名称推断功能完成度。
