...
> 状态：跨端实现参考；产品结构与唯一后端边界已由 ADR-002 更新
> 日期：2026-08-03
> 适用范围：HarmonyOS、Android、iOS 客户端及其共享工程边界
> 产品与设计基线：`PRODUCT.md`、`DESIGN.md`、`docs/product/`、`docs/design-system/视觉设计规范.md`
> 决策依据：`docs/architecture/ADR-001-文档真源与烟晶材质基线.md`

> [!WARNING]
> 本文中的“今日｜学习｜知识库”、全局学习地图和 `server/` 仅作为待契约化 BFF 的表述已经过时。现行结构与模块职责以 [ADR-002](ADR-002-统一产品口径与单一学习项目闭环.md) 和 [跨端架构与实现边界](../product/05-跨端架构与实现边界.md) 为准；本文继续用于原生体验、共享合同和平台适配方法参考。
...
- 当前 `admin/` React/Vite 工程保留已确认的浅灰紫连续知识场与烟晶悬浮控制，完成基准截图后冻结为高保真视觉与交互参考，不再作为移动端生产代码演进；
- `server/` 是 Web 原型和 HarmonyOS 共用的唯一业务后端与事实来源，当前接口仍需继续契约化并升级为生产实现；
...
2026-07-31 最终恢复了改版前的灰紫环境、烟晶页眉与导航、Agent 和覆盖层。当前正式样式仍约 8,700 行、214 KB，并继续使用 `LociGlass`、`GlassSurface` 和全局 CSS 覆盖；`admin/archive/glass/` 只保存不再进入正式路由的材质实验。现有视觉体验是迁移参考，但 DOM、CSS、SVG 位移滤镜与 Pointer Events 均不具备 ArkUI 或 React Native 的直接复用价值。
...
- 浅灰紫连续环境延伸到系统安全区，交互内容使用安全区 Padding；
...
- 正文、普通列表和阅读内容使用稳定 `Column` / `Row` / `Stack` 实体层、边界与语义 Token；
- 页眉、底部导航、短时筛选、Agent 和必要覆盖层通过语义化 `SmokeSurface` 适配器实现烟晶材质，不迁移 Web 的滤镜或 SVG 位移代码；
- `backgroundEffect`、`backgroundBlurStyle` 或其他平台材质能力只有在目标真机通过性能、可读性与降级验收后才可启用；能力不可用时回退到已定义的半透明着色或实体表面。
...
- 建立烟晶组件清单，逐项记录语义职责、Web 基准、ArkUI 原生映射、性能预算和实体降级态；
...
- 390 × 844px 基准截图的构图、色彩层级、烟晶控制层和主要尺寸与当前原型一致；
- 关闭实时模糊后的实体降级态仍满足信息层级、文字对比与交互可用性；
...
6. S09 学习地图变化；
7. S10 今日成果与下一次安排；
8. S07 Agent 抽屉的可选模拟态；
9. S01 欢迎注册与 S02 轻量个性化，在注册顺序开放决策确认后接入同一旅程。
...
- 减少动态下仍保持可理解；
- 所有烟晶覆盖面板在关闭实时模糊后仍保持层级和可读性，启用原生材质时满足目标真机性能预算。
...
- 不直接调用数据库、网络或系统 API；
- 有截图基准和至少一条核心流程测试；
- 烟晶组件有目标平台实现、真机性能记录和关闭实时模糊后的降级截图；
- 无隐藏的全局样式或祖先选择器依赖。
...
| 把烟晶视觉等同于 Web 滤镜代码 | 跨端分裂、性能不稳 | 共享材质语义与验收图，各端原生适配；强制实体降级态和真机性能门禁 |
