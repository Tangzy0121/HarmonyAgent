...
本 PRD 不重复规定全部视觉参数。组件尺寸、磨砂玻璃、动效、响应式与无障碍细节以《UI与交互规范.md》和《视觉设计规范.md》为准。
...
4. 《用户旅程.md》；
5. 《信息架构与页面结构.md》；
6. 《致队友：项目构想与设计空间.md》及旧仓库实现。

《信息架构与页面结构.md》中仍存在“今日｜知识库｜Agent”以及“地图并入知识库”的早期方案。当前统一口径是：
...
目标是验证浅矿物灰画布、深色编辑式层级、稳定不透明表面、克制桃色行动、抽屉阻力、地图惯性与自动吸附，不接后端。
...
- 一级页面共享浅矿物灰画布、深色内容和实体表面，不为不同功能切换主题；
- 页面主体采用开放编辑式排版与稳定不透明表面，不做 SaaS 卡片矩阵；
- 正式界面不使用环境光、背景模糊、折射玻璃或页面级渐变；
- 重要内容区域不套满屏卡片，普通列表优先使用分隔线；
- 底部导航使用深梅黑实体与浅色选中面；Agent 使用独立桃色入口和桃花标识；
...
- 视觉层级不得依赖背景模糊或自定义材质，跨端使用实体表面与边界直接实现；
- 长列表使用按需渲染；
- 后台任务与 UI 状态解耦。
...
- [ ] 页面主体不是大面积卡片矩阵；
- [ ] 正式页面没有环境光、背景模糊、折射玻璃或页面级渐变；
- [ ] 页面共享矿物灰画布、深色排版和实体表面，桃色行动与冷紫复习语义没有被滥用；
